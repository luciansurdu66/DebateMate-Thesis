...
const AdjudicatorList = () => {
    ...
    const fetchAdjudicators = async () => {
        try {
            const response = (await AdjudicatorService.getAdjudicators()).adjudicators;
            setAdjudicators(response);
        } catch (error) {
        ...
        <Menu />
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 hide-scrollbar">
            ...
            {adjudicators.filter(adjudicator => 
                ...
                adjudicator.club.toLowerCase().includes(searchTerm.toLowerCase())
              ).map(adjudicator => (
                <tr key={adjudicator.id} className="border-b border-gray-200 dark:border-gray-700">
                  <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{adjudicator.id}</td>
                  <td className="px-6 py-4">{adjudicator.name}</td>
                  <td className="px-6 py-4">{adjudicator.club}</td>
...
export default AdjudicatorList;
